...
VrArrayPtrF64 sum(VrArrayPtrF64 A) {
	...
	VR_GET_DATA_F64(outArr) = sum<double,dim_type>(VR_GET_DATA_F64(A),collapseDim,bucketSize,size,step);
	return outArr;
} 
...
VrArrayPtrF32 sum(VrArrayPtrF32 A) {
	...
	VR_GET_DATA_F32(outArr) = sum<float,dim_type>(VR_GET_DATA_F32(A),collapseDim,bucketSize,size,step);
	return outArr;
} 
VrArrayPtrCF32  sum(VrArrayPtrCF32 A) {
	...
	VR_GET_DATA_CF32(outArr) = sum<float complex,dim_type>(VR_GET_DATA_CF32(A),collapseDim,bucketSize,size,step);
	return outArr;
} 
VrArrayPtrCF64  sum(VrArrayPtrCF64 A) {
	...
	VR_GET_DATA_CF64(outArr) = sum<double complex,dim_type>(VR_GET_DATA_CF64(A),collapseDim,bucketSize,size,step);
	return outArr;
}
