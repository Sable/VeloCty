...
ArrayType vertcat(int nargs, va_list args) {
    ...
    //ArrayType outArr = getAllocFunc_vertcat<ArrayType,DimType>(ndims, 0, outDims);
    int ncols = outDims[1];
    int k =0;
    ...
    //for(int m = 0; m < hDims; m++ ) {
        //for(int i = 0; i < ncols; i++) {
            for(int j = 0; j < nargs; j++) {
                ...
                DimType numel = VR_GET_DIMS_F64(arr[j])[0] * VR_GET_DIMS_F64(arr[j])[1] * hDims;
                memcpy(VR_GET_DATA_F64(outArr) + k, (VR_GET_DATA_F64(arr[j])), sizeof(DataType) * numel);
                k += numel;
            }
       // }
    //}
