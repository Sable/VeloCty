...
/*VrArrayPtrCI64 vrAllocArrayI64CM(int ndims,int func,int *sizes){
	switch(func){
	case 0: //Zeros
		return mxCreateNumericArray(ndims,(int*)sizes,mxINT64_CLASS,mxCOMPLEX);
	case 1: // Ones
		VrArrayPtrCI64 arr=mxCreateNumericArray(ndims, (int*)sizes, mxINT64_CLASS,mxCOMPLEX);
	...
	switch(func){
	case 0: //Zeros
		VrArrayPtrCI64 arr=(VrArrayPtrCI64)VR_MALLOC(sizeof(VrArrayCI64));  //mxCreateNumericArray(ndims,(int*)sizes,mxSINGLE_CLASS,mxREAL);
		arr->data=(float*)VR_MALLOC(sizeof(float)*getNumElem(sizes,ndims));
		arr->ndims=ndims;
		//memcpy(arr->dims,sizes,ndims*sizeof(int));
		arr->dims=sizes;
		return arr;
	case 1: // Ones
		VrArrayPtrF32 arr=mxCreateNumericArray(ndims, (int*)sizes, mxSINGLE_CLASS,mxREAL);
		int numel=mxGetNumberOfElements(arr);
		float* data=(float*)mxGetPr(arr);
		
		VrArrayPtrF32 arr=(VrArrayPtrF32)VR_MALLOC(sizeof(VrArrayF32));  //mxCreateNumericArray(ndims,(int*)sizes,mxSINGLE_CLASS,mxREAL);
		arr->data=(float*)VR_MALLOC(sizeof(float));
		arr->ndims=ndims;
		//memcpy(arr->dims,sizes,ndims*sizeof(int));
		arr->dims=sizes;
		int numel=getNumElem(sizes,ndims);
...
VrArrayPtrCI32 vrAllocArrayI32CM(int ndims,int func,int *sizes){
	switch(func){
	case 0: //Zeros
		return mxCreateNumericArray(ndims,(int*)sizes,mxINT32_CLASS,mxCOMPLEX);
	case 1: // Ones
		VrArrayPtrCI32 arr=mxCreateNumericArray(ndims, (int*)sizes, mxINT32_CLASS,mxCOMPLEX);
		int numel=mxGetNumberOfElements(arr);
		int* data=(int*)mxGetData(arr);
		for(int i=0;i<numel;i++){
			data[i]=1;
